            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if cls < len(class_names):
                        ...
                        # 标签背景
                        draw.rectangle([x1, bg_y1, bg_x2, bg_y2], 
...
def generate_safety_analysis(detections):
    ...
    total_people = detections.get('Person', 0) + detections.get('NO-Hardhat', 0) + detections.get('NO-Mask', 0) + detections.get('NO-Safety Vest', 0)
...
if __name__ == '__main__':
    ...
    def get_rtsp_frame():
        ...
        try:
            if rtsp_current_frame is None:
                ...
                cv2.putText(placeholder, 'No RTSP Stream', (200, 240), 
    ...
    def detect_rtsp():
        ...
        try:
            ...
            # 计算缩放比例（如果进行了缩放）
            scale_x = frame_copy.shape[1] / detection_frame.shape[1] if detection_frame.shape[1] != frame_copy.shape[1] else 1.0
            scale_y = frame_copy.shape[0] / detection_frame.shape[0] if detection_frame.shape[0] != frame_copy.shape[0] else 1.0
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        if conf > confidence_threshold:  # 使用动态置信度阈值
                            ...
                            cv2.rectangle(detected_frame, (x1, y1 - label_size[1] - 8), 
                                        (x1 + label_size[0], y1), color_rgb, -1)
                            cv2.putText(detected_frame, label, (x1, y1 - 4), 
