...
class ImageDetectionManager {
    ...
    displaySafetyAnalysis(analysis) {
        ...
        // 清空占位符
        const placeholder = this.safetyAnalysis.querySelector('.safety-placeholder');
        if (placeholder) {
            placeholder.remove();
        }
    ...
    createHistoryItem(item) {
        ...
        const totalDetections = Object.values(item.detections || {}).reduce((sum, count) => sum + count, 0);
