...
class BatchDetectionManager {
    ...
    initializeElements() {
        ...
        this.batchProgressPercentage = document.getElementById('batch-progress-percentage');
        this.batchProgressFill = document.getElementById('batch-progress-fill');
        this.batchProgressDetails = document.getElementById('batch-progress-details');
        ...
        this.successfulDetections = document.getElementById('successful-detections');
    ...
    bindEvents() {
        ...
        if (this.batchFileInput) {
            this.batchFileInput.addEventListener('change', (e) => this.handleFileSelect(e));
        }
        ...
        if (this.startBatchBtn) {
            this.startBatchBtn.addEventListener('click', () => this.startBatchDetection());
        }
        
        // 过滤和排序
        if (this.safetyFilter) {
            this.safetyFilter.addEventListener('change', (e) => this.updateFilter('safety', e.target.value));
        }
        if (this.detectionFilter) {
            this.detectionFilter.addEventListener('change', (e) => this.updateFilter('detection', e.target.value));
        }
        if (this.sortOptionSelect) {
            this.sortOptionSelect.addEventListener('change', (e) => this.updateSort(e.target.value));
        }
        ...
        if (this.exportReportBtn) {
            this.exportReportBtn.addEventListener('click', () => this.exportDetectionReport());
        }
        if (this.clearResultsBtn) {
            this.clearResultsBtn.addEventListener('click', () => this.clearAllResults());
        }
    ...
    setupDragAndDrop() {
        ...
        events.forEach(eventName => {
            this.batchUploadArea.addEventListener(eventName, this.preventDefaults, false);
        });

        ['dragenter', 'dragover'].forEach(eventName => {
            this.batchUploadArea.addEventListener(eventName, () => {
    ...
    processSelectedFiles(files) {
        ...
        const imageFiles = files.filter(file => {
            ...
            if (!isValidSize) {
                console.warn('跳过过大文件:', file.name, this.formatFileSize(file.size));
            }
        ...
        if (imageFiles.length === 0) {
            this.showNotification('请选择有效的图片文件（JPG、PNG、JPEG，最大10MB）', 'error');
            return;
        }
        ...
        this.showNotification(`已添加 ${imageFiles.length} 个文件到队列`, 'success');
    ...
    async startBatchDetection() {
        ...
        try {
            ...
            for (let i = 0; i < this.currentFiles.length; i++) {
                ...
                this.updateProgress(progress, `正在处理: ${file.name} (${i + 1}/${this.currentFiles.length})`);
                this.updateProgressDetails(`已处理: ${this.processingStats.processed}, 成功: ${this.processingStats.successful}, 失败: ${this.processingStats.failed}`);
            ...
            this.updateProgressDetails(`总计: ${this.processingStats.total}, 成功: ${this.processingStats.successful}, 失败: ${this.processingStats.failed}`);
            
            // 显示结果
            this.displayBatchResults();
            this.showNotification(`批量检测完成！成功处理 ${this.processingStats.successful} 个文件`, 'success');
    ...
    calculateStats() {
        ...
        return {
            ...
            successfulDetections,
            safetyWarnings,
            avgSafetyScore: totalProcessed > 0 ? Math.round(totalSafetyScore / totalProcessed) : 0
        };
    }
    ...
    createResultItem(item) {
        ...
        // 检测统计
        const detectionCount = result.detections ? Object.values(result.detections).reduce((a, b) => a + b, 0) : 0;
        const detectionTypes = result.detections ? Object.keys(result.detections).length : 0;
    ...
    getFilteredResults() {
        return this.batchResults.filter(item => {
            ...
            if (this.filters.detection !== 'all') {
                const hasDetections = result.detections && Object.keys(result.detections).length > 0;
        ...
        return [...results].sort((a, b) => {
            ...
            switch (this.sortOption) {
                case 'filename':
                    ...
                    break;
                case 'safety-score':
                    compareValue = (a.result.analysis?.safety_score || 0) - (b.result.analysis?.safety_score || 0);
                    break;
                case 'detection-count':
                    const countA = a.result.detections ? Object.values(a.result.detections).reduce((sum, count) => sum + count, 0) : 0;
                    const countB = b.result.detections ? Object.values(b.result.detections).reduce((sum, count) => sum + count, 0) : 0;
    ...
    generateDetailContent(item) {
        ...
        const detectionStats = result.detections ? 
            ...
            ).join('') : '<div class="stat-item">未检测到目标</div>';
    ...
    updateFileStatus(index, status, text) {
        const fileItem = this.fileListContent?.querySelector(`[data-index="${index}"]`);
        if (!fileItem) return;
