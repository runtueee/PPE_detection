# evaluate_models.py
"""批量评估多个YOLO模型, 输出每类mAP@0.5对比表 用于分析小目标检测提升效果."""
...
def batch_evaluate_models(
    model_paths: list, data_yaml: str, output_file: str = "model_comparison.csv", split: str = "val"
):
    """
    批量评估多个模型并生成对比表.
    ...
    if len(df) > 1:
        ...
        for i in range(1, len(df)):
            ...
            print(
                f"  整体mAP@0.5: {baseline['overall_mAP@0.5']:.4f} → {current['overall_mAP@0.5']:.4f} (Δ{current['overall_mAP@0.5'] - baseline['overall_mAP@0.5']:+.4f})"
            )
